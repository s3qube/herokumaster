...
<form name="invoicesForm" method="post" action="<?= base_url() ?>invoices/exportInvoices">
	...
	<table border="0" class="searchInvoiceTable">
		...
		<tr class="searchInvoiceTable">
			...
			<? if (checkPerms('can_view_invoice_export_date')) { ?>
				<th class="searchInvoiceTable">Exported</td>
			<? } ?>
			<th class="searchInvoiceTable">Bill To</td>
	...
	?>
			...
			<tr class="searchInvoiceTable <?= $alt ?>">
				...
				<td class="searchInvoiceTable"><?= $i->currencysymbol ?><?= number_format($i->total,2) ?></td>
				<td class="searchInvoiceTable"><?= $i->referencenumber ?></td>
				<td class="searchInvoiceTable"><?= opmDate($i->createdate) ?></td>
				<? if (checkPerms('can_view_invoice_export_date')) { ?>
					<td class="searchInvoiceTable"><?= ($i->exportdate ? opmDate($i->exportdate) : null) ?></td>
				<? } ?>
				<td class="searchInvoiceTable"><strong><?= $i->companyname ?></strong></td>
	...
	<? if (checkPerms('can_export_invoices') && $args['statusid'] == $this->config->item('invStatusApproved')) { ?>
	
		<br />
		
			<div align="right" style="margin-right:10px;"><input type="submit" name="submitBtn" value="Export Selected Invoices" class="invoiceBtn" onmouseover="this.className='invoiceBtnHover'" onmouseout="this.className='invoiceBtn'"></div>
		
		<br />
	
	<? } ?>
	
	<? if (checkPerms('can_print_multiple_invoices')) { ?>
	
		<br />
		
			<div align="right" style="margin-right:10px;"><input type="submit" name="printMultipleBtn" value="Print Selected Invoices" class="invoiceBtn" onmouseover="this.className='invoiceBtnHover'" onmouseout="this.className='invoiceBtn'"></div>
